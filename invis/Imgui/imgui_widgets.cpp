...
static ImVec2           InputTextCalcTextSizeW1(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining = NULL, ImVec2* out_offset = NULL, bool stop_on_new_line = false);
//-------------------------------------------------------------------------
// [SECTION] Widgets: Text, etc.
//-------------------------------------------------------------------------
// - TextEx() [Internal]
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    const float width = GetWindowWidth() - 32;
    ...
    if (it_anim->second.active)
        it_anim->second.size += (size.x / 2.f) * g.IO.DeltaTime * 5.f;

    if (it_anim->second.alpha <= 0.1f)
        it_anim->second.size = 0.f;

    if (it_anim->second.size >= size.x / 2.f)
        it_anim->second.active = false;

    // Render
    window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(colors::button_rect), 2.f);
    window->DrawList->PushClipRect(rect.Min, rect.Max, true);
    window->DrawList->AddCircleFilled(it_anim->second.pos, it_anim->second.size, GetColorU32(colors::accent, 0.5f * it_anim->second.alpha));
    window->DrawList->PopClipRect();
    window->DrawList->AddRect(rect.Min, rect.Max, GetColorU32(colors::child_background), 0.f);
    window->DrawList->AddRect(rect.Min, rect.Max, GetColorU32(colors::button_outline), 2.f);
    PushStyleColor(ImGuiCol_Text, colors::button_text);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    ImRect bb = ImRect(bb_frame.Min, bb_frame.Max - ImVec2(0, 2));
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(colors::accent), style.ScrollbarRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect clickable(pos + ImVec2(width - 30, 0), pos + ImVec2(width, 16));
    ItemSize(rect, style.FramePadding.y);
    if (!ItemAdd(rect, id))
    {
    ...
    it_anim->second.rect_alpha = ImClamp(it_anim->second.rect_alpha + (8.f * g.IO.DeltaTime * (*v ? 1.f : -1.f)), 0.f, 1.f);
    it_anim->second.circle_offset = ImLerp(it_anim->second.circle_offset, *v ? 14.f : 0.f, g.IO.DeltaTime * 12.f);

    window->DrawList->AddText(rect.Min - ImVec2(1, 0), GetColorU32(colors::checkbox_text), label);
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(colors::checkbox_rect), 8.f);
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(colors::accent, it_anim->second.rect_alpha), 8.f);
    window->DrawList->AddCircleFilled(clickable.Min + ImVec2(8 + it_anim->second.circle_offset, 8), 5.f, GetColorU32(colors::checkbox_circle));
...
struct combo_state
{
    ...
    bool hovered = false;
    ImVec4 arrow_color = colors::combo_arrow;
};
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, ImGuiComboFlags flags, bool multi)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const float width = GetWindowWidth() - 32;
    const ImRect rect(pos, pos + ImVec2(width, 48));
    const ImRect clickable(pos + ImVec2(0, 18), pos + ImVec2(width, 48));
    ItemSize(rect, style.FramePadding.y);
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 24) + (8) : 0.f, g.IO.DeltaTime * 12.f);
    it_anim->second.arrow_color = ImLerp(it_anim->second.arrow_color, it_anim->second.opened_combo ? colors::combo_text : colors::combo_arrow, g.IO.DeltaTime * 12.f);

    // Render shape
    window->DrawList->AddText(rect.Min - ImVec2(1, 4), GetColorU32(colors::combo_text), label);
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(colors::combo_rect), 2.f);
    window->DrawList->AddRect(clickable.Min, clickable.Max, GetColorU32(colors::combo_outline), 2.f);
    PushStyleColor(ImGuiCol_Text, colors::combo_text);
    RenderTextClipped(clickable.Min - ImVec2(-15, 2), clickable.Max - ImVec2(0, 2), preview_value, NULL, NULL, ImVec2(0.f, 0.5f));
    PopStyleColor();
    ImRotateStart();
    window->DrawList->AddText(arrow, 10.f, rect.Max - ImVec2(22, 20), GetColorU32(it_anim->second.arrow_color), "A");
    ...
    ImGui::SetNextWindowPos(ImVec2(clickable.Min.x + 2, clickable.Max.y - 1));
    ImGui::SetNextWindowSize(ImVec2(clickable.GetWidth() - 4, it_anim->second.combo_size));
    ...
    PushStyleColor(ImGuiCol_WindowBg, colors::combo_window);
    PushStyleColor(ImGuiCol_Border, colors::combo_outline);
    ...
    if (!multi)
        if (it_anim->second.hovered && g.IO.MouseClicked[0])
            it_anim->second.opened_combo = false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const float width = GetWindowWidth() - 32;
    const ImRect rect(pos, pos + ImVec2(width, 30));
    const ImRect clickable(pos + ImVec2(0, 18), pos + ImVec2(width, 30));
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(ImRect(clickable.Min - ImVec2(3, 0), clickable.Max + ImVec2(2, 0)), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    it_anim->second = ImLerp(it_anim->second, grab_bb.Min.x - clickable.Min.x, g.IO.DeltaTime * 20.f);

    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0, 3), clickable.Max - ImVec2(0, 3), GetColorU32(colors::slider_rect), 2.f);
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0, 3), ImVec2(clickable.Min.x + it_anim->second + 7, clickable.Max.y - 3), GetColorU32(colors::accent), 2.f);
    window->DrawList->AddCircleFilled(ImVec2(clickable.Min.x + it_anim->second + 6, clickable.Max.y - 6), 6.f, GetColorU32(colors::slider_circle));
    window->DrawList->AddText(rect.Min - ImVec2(1, 4), GetColorU32(colors::slider_text), label);
    PushStyleColor(ImGuiCol_Text, colors::slider_value);
    RenderTextClipped(rect.Min - ImVec2(0, 4), rect.Max, value_buf, value_buf_end, NULL, ImVec2(1.f, 0.f));
    PopStyleColor();
...
static ImVec2 InputTextCalcTextSizeW1(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ImGuiContext& g = *GImGui;
    ImFont* font = g.Font;
    const float line_height = g.FontSize;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect rect(frame_bb.Min, frame_bb.Min + total_size + ImVec2(0, 17));
    ...
    if (g.ActiveId == id)
    {
        ...
        // Edit in progress
        const float mouse_x = (io.MousePos.x - rect.Min.x - style.FramePadding.x) + state->ScrollX - 10;
    ...
    if (!is_multiline)
    {
        window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(colors::input_rect), 2.f);
        if (strlen(buf_display) == 0)
            window->DrawList->AddText(rect.Min + ImVec2(15, 7), GetColorU32(colors::input_text), label);
    }
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : rect.Min + style.FramePadding + ImVec2(10, 4);
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            cursor_offset.x = InputTextCalcTextSizeW1(ImStrbolW(searches_input_ptr[0], text_begin), searches_input_ptr[0]).x;
            cursor_offset.y = searches_result_line_no[0] * g.FontSize;
            if (searches_result_line_no[1] >= 0)
            {
                select_start_offset.x = InputTextCalcTextSizeW1(ImStrbolW(searches_input_ptr[1], text_begin), searches_input_ptr[1]).x;
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                {
                    ImVec2 rect_size = InputTextCalcTextSizeW1(p, text_selected_end, &p, NULL, true);
                    if (rect_size.x <= 0.0f) rect_size.x = IM_FLOOR(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty lines
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - g.FontSize), rect_pos + ImVec2(rect_size.x, bg_offy_dn));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(0, 1), rect.Max + ImVec2(0, 2), GetColorU32(colors::accent, 0.5f), 2.f);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, GetColorU32(colors::input_buf), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            it_anim->second = ImLerp(it_anim->second, cursor_screen_rect.Min.x - rect.Min.x, g.IO.DeltaTime * 20.f);
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(rect.Min + ImVec2(it_anim->second, 8), rect.GetBL() - ImVec2(-it_anim->second, 8), GetColorU32(ImGuiCol_Text));
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, GetColorU32(colors::input_buf), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ItemHoverable(g.LastItemData.Rect, g.LastItemData.ID) && g.IO.MouseClicked[0] || it_anim->second.active && g.IO.MouseClicked[0] && !it_anim->second.hovered)
        ...
        window_name += label;
        SetNextWindowPos(g.LastItemData.Rect.GetBR() - ImVec2(202, -5));
        SetNextWindowSize(ImVec2(202, flags & ImGuiColorEditFlags_AlphaBar ? 159 : 146));
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
        PushStyleVar(ImGuiStyleVar_WindowRounding, 2.f);
        PushStyleColor(ImGuiCol_WindowBg, colors::color_rect);
        PushStyleColor(ImGuiCol_Border, colors::color_outline);
        PushStyleVar(ImGuiStyleVar_WindowBorderSize, 1.f);
        PushStyleVar(ImGuiStyleVar_Alpha, it_anim->second.alpha);
        if (it_anim->second.alpha >= 0.01f);
        {
            Begin(window_name.c_str(), NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove);
            {
                it_anim->second.hovered = IsWindowHovered();
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 picker_pos = window->DC.CursorPos + ImVec2(6, 6);
    ...
    // SV rectangle logic
    SetCursorPos(GetCursorPos() + ImVec2(6, 6));
    InvisibleButton("sv", ImVec2(picker_size, picker_size - 70));
    if (IsItemActive())
    {
        S = ImSaturate((io.MousePos.x - picker_pos.x) / (picker_size - 1));
        V = 1.0f - ImSaturate((io.MousePos.y - picker_pos.y) / (picker_size - 1 - 70));
    ...
    // Hue bar logic
    SetCursorScreenPos(ImVec2(picker_pos.x, picker_pos.y + 124.f));
    ...
    if (alpha_bar)
    {
        SetCursorScreenPos(ImVec2(picker_pos.x, picker_pos.y + 137.f));
    ...
    ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(R, G, B, style.Alpha)); // Important: this is still includ7ing the main rendering/style alpha!!

    ImVec2 sv_cursor_pos;

    // Render SV Square
    const int vtx_idx_0 = draw_list->VtxBuffer.Size;
    draw_list->AddRectFilled(picker_pos, picker_pos + ImVec2(picker_size, picker_size - 2 - 70), col_white, 2.0f);
    ...
    draw_list->AddRectFilledMultiColor2(picker_pos, picker_pos + ImVec2(picker_size, picker_size - 70), 0, 0, col_black, col_black, 2.f);

    sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * picker_size), picker_pos.x + 2, picker_pos.x + picker_size - 2); // Sneakily prevent the circle to stick out too much
    sv_cursor_pos.y = ImClamp(IM_ROUND(picker_pos.y + ImSaturate(1 - V) * (picker_size - 70)), picker_pos.y + 2, picker_pos.y + picker_size - 2 - 70);

    // Render Hue Bar
    for (int i = 0; i < 6; ++i)
        draw_list->AddRectFilledMultiColor2(ImVec2(picker_pos.x + i * (bar_width / 6) - (i == 5 ? 1 : 0), picker_pos.y + 128), ImVec2(picker_pos.x + (i + 1) * (bar_width / 6) + (i == 0 ? 1 : 0), picker_pos.y + 128 + bar_height), col_hues[i], col_hues[i + 1], col_hues[i + 1], col_hues[i], 2.f, i == 0 ? ImDrawFlags_RoundCornersLeft : i == 5 ? ImDrawFlags_RoundCornersRight : ImDrawFlags_RoundCornersNone);
    ...
    draw_list->AddCircleFilled(ImVec2(bar0_line_x, picker_pos.y + 130), 4, GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColor2(picker_pos + ImVec2(0, 141), picker_pos + ImVec2(bar_width, 141 + bar_height), col_black, user_col32_striped_of_alpha, user_col32_striped_of_alpha, col_black, 2.f);
        ...
        draw_list->AddCircleFilled(ImVec2(bar1_line_x, picker_pos.y + 143), 4, GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->AddText(rect.Min + ImVec2(-1, 1), GetColorU32(colors::color_text), desc_id);
    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max, GetColorU32(col_rgb), 9.f, ImVec2(0.f, 0.f), 2.f);
...
{
    ImVec4 text_color = colors::selectable_text_default;
    float text_offset = 0.f;
    float circle_alpha = 0.f;
};
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? colors::selectable_text_active : colors::selectable_text_default, g.IO.DeltaTime * 8.f);
    it_anim->second.text_offset = ImLerp(it_anim->second.text_offset, selected ? 9.9f : 0.f, g.IO.DeltaTime * 8.f);
    it_anim->second.circle_alpha = ImClamp(it_anim->second.circle_alpha + (4.f * g.IO.DeltaTime * (selected ? 1.f : -1.f)), 0.f, 1.f);

    window->DrawList->AddCircleFilled(bb.Min + ImVec2(15, 12), 2.f, GetColorU32(colors::accent, it_anim->second.circle_alpha));
    PushStyleColor(ImGuiCol_Text, it_anim->second.text_color);
    RenderTextClipped(text_min - ImVec2(-5 - it_anim->second.text_offset, 3), text_max - ImVec2(0, 3), label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
...
struct tab_state
{
    ImVec4 text_color = colors::tab_text_default;
    float rect_alpha = 0.f;
};

bool ImGui::Tab(const char* label, ImTextureID icon, bool active, const char* override_id)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(override_id == "" ? label : override_id);

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos + ImVec2(145, 32));
    ItemSize(rect, style.FramePadding.y);
    if (!ItemAdd(rect, id))
        return false;
    ...
    it_anim->second.rect_alpha = ImClamp(it_anim->second.rect_alpha + (5.f * g.IO.DeltaTime * (active ? 1.f : -1.f)), 0.f, 1.f);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, active ? colors::tab_text_active : hovered ? colors::tab_text_hovered : colors::tab_text_default, g.IO.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(ImVec4(1.f, 1.f, 1.f, 0.04f * it_anim->second.rect_alpha)), 2.f);
    window->DrawList->AddRectFilled(rect.Min + ImVec2(0, 5), ImVec2(rect.Min.x + 2, rect.Max.y - 5), GetColorU32(colors::accent, it_anim->second.rect_alpha));
    window->DrawList->AddImage(icon, rect.Min + ImVec2(15, 9), rect.Min + ImVec2(29, 23), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.text_color));
    window->DrawList->AddText(rect.Min + ImVec2(38, 8), GetColorU32(it_anim->second.text_color), label);
...
void ImGui::MultiCombo(const char* label, bool variable[], const char* labels[], int count)
{
    ...
    if (BeginCombo(label, preview.c_str(), count, 0, true)) // draw start
    {
...
bool ImGui::Keybind(const char* label, int* key, int* mode)
{
    ...
    const ImGuiID id = window->GetID(label);
    const float width = GetWindowWidth() - 32.f;
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(colors::key_rect), 2.f);
    window->DrawList->AddRect(clickable.Min, clickable.Max, GetColorU32(colors::key_outline), 2.f);
    window->DrawList->AddText(rect.Min - ImVec2(1, -1), GetColorU32(colors::key_text), label);
    PushStyleColor(ImGuiCol_Text, colors::key_text);
    RenderTextClipped(clickable.Min - ImVec2(0, 2), clickable.Max - ImVec2(0, 2), buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
